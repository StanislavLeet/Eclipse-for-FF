...
def _is_missing_deletion_tables_error(exc: ProgrammingError) -> bool:
    message = str(exc).lower()
    return (
        "game_deletion_requests" in message
        and ("does not exist" in message or "no such table" in message or "undefinedtable" in message)
    )
...
async def get_game_deletion_request(db: AsyncSession, game_id: int) -> GameDeletionRequest | None:
    try:
        ...
        if _is_missing_deletion_tables_error(exc):
            logger.warning("Game deletion tables are missing, skipping deletion status lookup")
            return None
        raise
    return result.scalar_one_or_none()
...
async def get_game_deletion_approvals(
    ...
    try:
        ...
        if _is_missing_deletion_tables_error(exc):
            logger.warning("Game deletion tables are missing, skipping deletion approvals lookup")
            return []
        raise
    return list(result.scalars().all())
